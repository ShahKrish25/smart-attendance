      ...
      if (conflictingSessions.length > 0) {
        ...
        return res.status(400).json({
          ...
          conflicts: conflictingSessions.map((session) => ({
            ...
            room: session.room,
            type: session.instructor.equals(classData.instructor._id)
              ? "instructor"
              : "room",
          })),
        });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.user.role !== "admin" &&
        !existingSession.instructor.equals(req.userId)
      ) {
        return res.status(403).json({
          success: false,
      ...
      const conflictingSessions = await ClassSession.find({
        ...
        $or: [{ instructor: classData.instructor._id }, { room: room }],
        status: { $ne: "cancelled" },
        $and: [
          { scheduledStartTime: { $lt: scheduledEndTime } },
          { scheduledEndTime: { $gt: scheduledStartTime } },
  ...
  try {
    ...
    if (session.status === "active") {
      return res.status(400).json({
        success: false,
        message:
          "Cannot delete an active session. Please end the session first.",
      });
    }
...
router.post("/:id/attendance", auth, async (req, res) => {
  try {
    ...
    if (existingRecord) {
      return res.status(400).json({
        success: false,
        message: `Attendance already marked as "${
          existingRecord.status
        }" at ${existingRecord.markedAt.toLocaleTimeString()}`,
      });
    }
